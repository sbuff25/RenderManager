...
def create_job_card(job):
    ...
    # v2.25.2 — cards live in a SortableJS list (see main.py): drag to
    # rearrange with smooth animation. Purely cosmetic - status/claiming
    # are never affected by card position.
    with ui.card().classes('w-full job-card-draggable').style(f'border-left: 3px solid {engine_color};'):
