...
Entry point for running as a package: python -m wane
...
# ============================================================================
def run():
    """Run the Wane application."""
    mode = "Desktop (Native)" if HAS_NATIVE_MODE else "Browser"
    print(f"Starting Wane ({mode} Mode)...")
    ...
    possible_asset_dirs = [
        os.path.join(parent_dir, 'assets'),     # ../assets (next to wane package)
        os.path.join(cwd, 'assets'),            # ./assets (current directory)
        os.path.join(package_dir, 'assets'),    # wane/assets (inside package)
    ]
    ...
    icon_ico = os.path.join(assets_dir, 'wane_icon.ico')
    icon_png = os.path.join(assets_dir, 'wane_logo.png')
    ...
    if HAS_NATIVE_MODE:
        ...
        app.native.window_args['title'] = 'Wane'
        ...
        class WindowAPI:
            ...
            def _get_hwnd(self):
                ...
                try:
                    import ctypes
                    return ctypes.windll.user32.FindWindowW(None, 'Wane')
                except:
                    return None
        ...
        if sys.platform == 'win32':
            ...
            def set_taskbar_icon_windows():
                ...
                try:
                    ...
                    hwnd = user32.FindWindowW(None, 'Wane')
                    if hwnd == 0:
                        print("Could not find Wane window for icon")
                        return
